...
const formatTime = function formatTime(time, template) {
    ...
    if (/^\d{8}$/.test(time)) {
        ...
        arr.push($1, $2, $3);  // 分开年月日出来
    } else {
        arr = time.match(/\d+/g); // 分开全部时间 年月日时分秒
    }
    
    // console.log(arr);  //['2023', '11', '13', '09', '14', '27']
    // replace 替换 将{0} {1} {2} 这些替换为对应的年月日
    return template.replace(/\{(\d+)\}/g, (_, $1) => {
        let item = arr[$1] || "00";
        if (item.length < 2) item = "0" + item;
        // console.log(item);  // 2023 11  13 (打印三次)
        return item;
    });
};
